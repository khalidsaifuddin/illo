...
class TimMarketing extends Component {
    state = {
        error: null,
        loading: true,
        routeParams:{
    ...
    componentDidMount = () => {
        ...
        this.props.getTimMarketing(this.state.routeParams).then((result)=>{
            this.setState({
                tim_marketing: result.payload
            })
        })
    ...
    tambah = () => {
        // alert('tes')
        this.$f7router.navigate('/FormTimMarketing/')
    }
    ...
    hapus = (option) => {
        ...
        this.$f7.dialog.confirm('Apakah Anda yakin ingin menghapus '+option.nama+' sebagai tim marketing?', 'Konfirmasi', ()=>{
            
            this.$f7.dialog.preloader('Menghapus data...')

            this.props.simpanTimMarketing({
                tim_marketing_id: option.tim_marketing_id, 
    ...
    nonaktifkan = (option, aktif) => {
        ...
        this.$f7.dialog.confirm('Apakah Anda yakin ingin '+(parseInt(aktif) === 0 ? 'menonaktifkan' : 'mengaktifkan')+' '+option.nama+' sebagai tim marketing?', 'Konfirmasi', ()=>{
    ...
    {
        return (
            <Page name="TimMarketing" hideBarsOnScroll>
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    <NavTitle sliding>Tim Marketing</NavTitle>
                </Navbar>
                ...
                <Popup opened={this.state.popupOpen} className="demo-popup-swipe-handler popupLebar" swipeToClose swipeHandler=".swipe-handler" onPopupClosed={()=>this.setState({popupOpen:false})}>
                    <Page>
                        <Navbar className="swipe-handler" title={"Filter Tim Marketing"}>
                            <NavRight>
                        ...
                        <Block strong style={{marginTop:'0px', padding:'0px'}}>
                            ...
                            <List style={{marginTop:'0px', marginBottom:'0px'}}>
                                <Searchbar
                                    ...
                                    placeholder="Cari Tim Marketing..."
                                        ...
                                        <div className="data-table" style={{overflowY:'hidden'}}>
                                            ...
                                            <div className="data-table-footer" style={{display:'block'}}>
                                                <div className="data-table-pagination" style={{textAlign:'right'}}>
                                                    ...
                                                    <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+this.state.routeParams.limit) >= parseInt(this.state.tim_marketing.total) ? "disabled" : "" )}>
                                                        <i className="icon icon-next color-gray"></i>
                                                    </a>
                                                    {this.state.tim_marketing.total > 0 &&
                                                    <span className="data-table-pagination-label">{(this.state.tim_marketing.total > 0 ? (this.state.routeParams.start+1) : 0)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit) <= parseInt(this.state.tim_marketing.total) ? (this.state.routeParams.start)+parseInt(this.state.routeParams.limit) : parseInt(this.state.tim_marketing.total)} dari {this.formatAngka(this.state.tim_marketing.total)} Tiket</span>
                                                    }
                                                    {this.state.tim_marketing.total < 1 &&
                                                    <span className="data-table-pagination-label">Tidak ada tim marketing</span>
                                                    }
                                                </div>
                                            </div>
                                            <div style={{display:'block'}}>
                                                ...
                                                {this.state.tim_marketing.rows.map((option)=>{
                                                    ...
                                                    return (
                                                        <Card key={option.tim_marketing_id} style={{marginLeft:'0px', marginRight:'0px'}}>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getTimMarketing: Actions.getTimMarketing,
      simpanTimMarketing: Actions.simpanTimMarketing
    }, dispatch);
}
...
export default (connect(mapStateToProps, mapDispatchToProps)(TimMarketing));
