...
class DiskonProduk extends Component {
    ...
    edit = (diskon_produk_id) => {
        this.$f7router.navigate("/FormDiskonProduk/"+diskon_produk_id)
    }
    ...
    hapus = (diskon_produk) => {
        this.$f7.dialog.confirm('Apakah Anda yakin ingin menghapus diskon ini?', 'Konfirmasi', ()=>{
            ...
            this.props.simpanDiskonProduk({...diskon_produk, soft_delete: 1}).then((result)=>{
                this.$f7.dialog.close()
    ...
    {
        return (
            <Page name="DiskonProduk" className="halamanJenisTiket" hideBarsOnScroll style={{paddingBottom:'100px', boxSizing:'content-box'}}>
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    <NavTitle sliding>Diskon Produk</NavTitle>
                </Navbar>
                        ...
                        <Card noShadow noBorder style={{marginBottom:'50px', background: 'transparent'}}>
                            <CardContent style={{padding:'4px'}}>
                                <div style={{width:'100%', textAlign:'right'}}>
                                    <Button onClick={()=>this.$f7router.navigate('/FormDiskonProduk/')} raised fill style={{display:'inline-flex', marginBottom:'4px'}}>
                                ...
                                <div className="data-table" style={{overflowY:'hidden', paddingBottom:'32px'}}>
                                    <div className="data-table-footer" style={{display:'block'}}>
                                        <div className="data-table-pagination" style={{textAlign:'right'}}>
                                            ...
                                            <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+20) >= parseInt(this.state.diskon_produk.total) ? "disabled" : "" )}>
                                                <i className="icon icon-next color-gray"></i>
                                            </a>
                                            <span className="data-table-pagination-label">{(this.state.routeParams.start+1)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit) <= parseInt(this.state.diskon_produk.total) ? (this.state.routeParams.start)+parseInt(this.state.routeParams.limit) : parseInt(this.state.diskon_produk.total)} dari {this.formatAngka(this.state.diskon_produk.total)} Diskon Produk</span>
                                        </div>
                                    </div>
                                    <table>
                                        <thead style={{background:'#eeeeee'}}>
                                            <tr>
                                                ...
                                                <th className="label-cell" style={{minWidth:'200px', color:'#434343', fontSize:'13px'}}>Nama Produk</th>
                                        ...
                                        <tbody>
                                        {this.state.diskon_produk.rows.map((option)=>{
                                            ...
                                            return(
                                                <tr key={option.diskon_produk_id}>
                                                    <td className="label-cell">
                                                        <Button popoverOpen={".popover-menu-"+option.diskon_produk_id}><i className="icons f7-icons" style={{fontSize:'20px'}}>ellipsis_vertical</i></Button>
                                                        <Popover className={"popover-menu-"+option.diskon_produk_id} style={{minWidth:'300px'}}>
                                                            <List>
                                                                <ListItem link="#" popoverClose title="Edit" onClick={()=>this.edit(option.diskon_produk_id)} />
...
export default (connect(mapStateToProps, mapDispatchToProps)(DiskonProduk));
